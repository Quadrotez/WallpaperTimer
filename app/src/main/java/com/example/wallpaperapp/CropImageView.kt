...
class CropImageView @JvmOverloads constructor(
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        bitmap?.let { bmp ->
            ...
            val px = imageRect.width() * 0.1f; val py = imageRect.height() * 0.1f
            cropRect = RectF(imageRect.left + px, imageRect.top + py, imageRect.right - px, imageRect.bottom - py)
        }
    }
    ...
    private fun applyDrag(dx: Float, dy: Float) {
        ...
        when (activeHandle) {
            ...
            Handle.TL -> {
                cropRect.left = max(imageRect.left, min(cropRect.left + dx, cropRect.right - min))
                cropRect.top  = max(imageRect.top,  min(cropRect.top  + dy, cropRect.bottom - min))
            }
            Handle.TR -> {
                cropRect.right = min(imageRect.right, max(cropRect.right + dx, cropRect.left + min))
                cropRect.top   = max(imageRect.top,   min(cropRect.top   + dy, cropRect.bottom - min))
            }
            Handle.BL -> {
                cropRect.left   = max(imageRect.left,   min(cropRect.left   + dx, cropRect.right - min))
                cropRect.bottom = min(imageRect.bottom, max(cropRect.bottom + dy, cropRect.top   + min))
            }
            Handle.BR -> {
                cropRect.right  = min(imageRect.right,  max(cropRect.right  + dx, cropRect.left + min))
                cropRect.bottom = min(imageRect.bottom, max(cropRect.bottom + dy, cropRect.top  + min))
